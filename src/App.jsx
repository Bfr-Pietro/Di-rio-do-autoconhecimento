...
// ─── GEMINI SERVICE ───────────────────────────────────────────────────────────
const GEMINI_API_KEY = "AIzaSyDFrxzQw4coMX7IdhMi2celRZ9sO5IEDXo";
...
function DiaryPage({ dark, entries, setEntries, onAnalyze, loading }) {
  ...
  const saveEntry = async (entry) => {
    ...
    try {
      ...
      setSaveStatus("saved");
      setTimeout(() => setSaveStatus(null), 2000);
      setTimeout(() => onAnalyze(entries), 500);
    } catch {
      setSaveStatus("error");
    }
  };
...
function AppInner() {
  ...
  useEffect(() => {
    if (!user) return;
    FirebaseService.getAnalysis(user.uid).then((a) => {
      if (a) {
  ...
  // Roda análise quando as entradas carregam (só se não tiver análise salva)
  useEffect(() => {
    if (!entriesLoading && entries.length && !aiPatterns) {
      runAnalysis(entries);
    }
  }, [entriesLoading]);
